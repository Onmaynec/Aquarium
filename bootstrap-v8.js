(async()=>{
  ...
  const fail=error=>{console.error(error);status.className='error';status.textContent='Не удалось запустить Aquarium V8.\n\n'+(error?.message||error)};
  try{
    ...
    html=html.replace(/<title>[^<]*<\/title>/,'<title>Living Aquarium V8 Marine Genetics Lab</title>');
    html=html.replace('<link rel="stylesheet" href="styles.css">','<style>'+css+'</style>');
    html=html.replace('<script src="app.js"></script>','<script>'+app+'</script><script src="v7-expedition.js"></script><script src="v8-genetics.js"></script>');
