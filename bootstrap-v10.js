(async()=>{
  ...
  const fail=error=>{
    ...
    status.textContent='Не удалось запустить Aquarium V10.\n\n'+(error?.message||error);
  };
  try{
    ...
    const binary=atob(window.__AQ_PAYLOAD.join(''));
    ...
    for(let offset=0;offset+512<=tar.length;){
      ...
      const name=decoder.decode(header.subarray(0,100)).replace(/\0.*$/s,'').replace(/^\.\//,'');
    ...
    const text=name=>{
      ...
      if(!value)throw new Error('В payload отсутствует '+name);
    ...
    html=html.replace(/<title>[^<]*<\/title>/,'<title>Living Aquarium V10 Ocean Command Center</title>');
    html=html.replace('<link rel="stylesheet" href="styles.css">','<style>'+css+'</style>');
    html=html.replace(
      '<script src="app.js"></script>',
      '<script>'+app+'</script><script src="v7-expedition.js"></script><script src="v8-genetics.js"></script><script src="v9-restoration.js"></script><script src="v10-command.js"></script>'
    );
    document.open();
    document.write(html);
    document.close();
