...
for(const name of ['v7-expedition.js','v8-genetics.js','bootstrap-v8.js'])new vm.Script(fs.readFileSync(path.join(root,name),'utf8'),{filename:name});
const html=fs.readFileSync(path.join(root,'v8.html'),'utf8');
for(const marker of ['bootstrap-v8.js','runtime/chunk-00.js','Living Aquarium V8 Marine Genetics Lab'])if(!html.includes(marker))throw new Error(`Missing HTML marker: ${marker}`);
console.log(JSON.stringify({ok:true,payloadBytes:packed.length,coreFiles:files.size,overlays:['v7','v8']},null,2));
