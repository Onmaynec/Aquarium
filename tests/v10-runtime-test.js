...
const payload=chunks.map(text=>{
  const match=text.match(/push\('([A-Za-z0-9+/=]+)'\)/);
  if(!match)throw new Error('Invalid runtime chunk');
  return match[1];
}).join('');
...
for(let offset=0;offset+512<=tar.length;){
  ...
  offset+=512;
  if(name&&type!=='5')files.set(name,tar.subarray(offset,offset+size));
...
for(const name of ['v7-expedition.js','v8-genetics.js','v9-restoration.js','v10-command.js','bootstrap-v10.js','service-worker-v10.js']){
  new vm.Script(fs.readFileSync(path.join(root,name),'utf8'),{filename:name});
}
const html=fs.readFileSync(path.join(root,'v10.html'),'utf8');
for(const marker of ['bootstrap-v10.js','runtime/chunk-00.js','manifest-v10.webmanifest','Living Aquarium V10 Ocean Command Center']){
  if(!html.includes(marker))throw new Error(`Missing HTML marker: ${marker}`);
}
const manifest=JSON.parse(fs.readFileSync(path.join(root,'manifest-v10.webmanifest'),'utf8'));
if(manifest.name!=='Living Aquarium V10 Ocean Command Center')throw new Error('Unexpected V10 manifest name');
console.log(JSON.stringify({ok:true,payloadBytes:packed.length,coreFiles:files.size,overlays:['v7','v8','v9','v10']},null,2));
