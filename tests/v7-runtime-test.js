...
for(const test of ['integrity-test.js','ocean-lab-test.js']){const current=path.join(root,'tests',test);if(fs.existsSync(current))fs.copyFileSync(current,path.join(tmp,'tests',test))}
